...
import dataset_graph as dsg      # <-- NEW
...
def build_dataloaders(train_files, val_files, seq_length, batch_size, n_train, n_val, token_to_idx):
    ...
    train_ds = dsg.SMILESGraphDataset(train_smiles, seq_length=seq_length, token_to_idx=token_to_idx, augmentor=augmentor, augment_train=True)
    val_ds = dsg.SMILESGraphDataset(val_smiles, seq_length=seq_length, token_to_idx=token_to_idx, augmentor=None, augment_train=False)

    train_dl = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=8, pin_memory=True, prefetch_factor=2, collate_fn=dsg.collate_graph_smiles)
    val_dl = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=8, pin_memory=True, prefetch_factor=2, collate_fn=dsg.collate_graph_smiles)
...
def train_one_epoch(model, loader, optimizer, scaler, pad_idx, eos_idx, kl_w, device, corruption_p, label_smoothing, clip_grad, use_amp):
    ...
    for batch in loader:
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            loss, _, _ = met.vae_loss(logits, tgt, mu, logvar, padding_idx=pad_idx, eos_idx=eos_idx, kl_weight=kl_w, label_smoothing=label_smoothing, eos_weight=4.0)
...
def validate_tf(model, loader, pad_idx, eos_idx, kl_w, device, label_smoothing, use_amp):
    ...
    for batch in loader:
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            loss, _, _ = met.vae_loss(logits, tgt, mu, logvar, padding_idx=pad_idx, eos_idx=eos_idx, kl_weight=kl_w, label_smoothing=0.00, eos_weight=4.0)
        total_loss += loss.item()
        corr, tok = met.compute_accuracy_counts(logits, tgt, padding_idx=pad_idx, eos_idx=eos_idx, stop_at_eos=True)
        total_corr += corr
        total_tok  += tok
...
def run_training(cfg, token_to_idx, idx_to_token):
    required = [
        ...
        "d_model","latent_dim","enc_heads", "dec_heads", "dec_layers","enc_layers","ff_dim","dropout",
    ...
    print("Trainable params:", du.count_parameters(base_model))
    ...
    def kl_weight(ep):
        ...
        plateau = cfg.get("kl_plateau_until", 25)
