...
def vae_loss(pred_logits, target, mu, logvar, padding_idx=0, eos_idx=None, kl_weight=1.0, label_smoothing: float = 0.0, eos_weight: float = 4.0):
    tgt = target[:, 1:]
    ...
    if mask.any():
        ...
        if eos_idx is not None and eos_weight is not None and eos_weight > 1.0:
            eos_mask = (tgt == eos_idx)
            nll_tok = torch.where(eos_mask, nll_tok * eos_weight, nll_tok)
...
def chemical_validity_ratio(recon_smiles: list[str]) -> float:
    ...
    return valid / max(len(recon_smiles), 1)
