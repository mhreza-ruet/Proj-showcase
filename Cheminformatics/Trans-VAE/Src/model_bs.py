...
class EncoderTransformer(nn.Module):
    def __init__(self, vocab_size: int, d_model: int, pad_idx: int, enc_layers: int = 6, nhead: int = 8,
                 dim_feedforward: int = 1024, dropout: float = 0.1, emb_dropout: float = 0.1, max_len: int = 512):
        super().__init__()
        ...
        single_layer_enc = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward, dropout=dropout, batch_first=True, norm_first=True)
        self.encoder = nn.TransformerEncoder(single_layer_enc, num_layers=enc_layers)
    ...
    def forward(self, src: torch.Tensor):
        B, T = src.size()
        mask_tok = (src != self.pad_idx).float()
        src_key_padding_mask = (src == self.pad_idx)
        x = self.emb(src)
        x = self.emb_do(self.emb_ln(x))
        x = x + self.pe[:, :T, :].requires_grad_(False)
        x = self.pe_do(x)
        seq_out = self.encoder(x, src_key_padding_mask=src_key_padding_mask)
        seq_out = self.seq_ln(self.out_do(seq_out))
        lengths = mask_tok.sum(1).clamp(min=1)
        pooled = (seq_out * mask_tok.unsqueeze(-1)).sum(1) / lengths.unsqueeze(-1)
...
class TransformerDecoder(nn.Module):
    ...
    # ---------- Full Model ----------
class Trans_VAE(nn.Module):
    def __init__(self, vocab_size, d_model, latent_dim, pad_idx, sos_idx, eos_idx, enc_layers=7, dec_layers=7, nhead=8, dropout=0.1, emb_dropout=0.1, max_len=512, dim_feedforward: int | None = None):
        super().__init__()
        self.pad_idx, self.sos_idx, self.eos_idx = pad_idx, sos_idx, eos_idx
        self.d_model     = d_model
        self.latent_dim  = latent_dim
        self.encoder = EncoderTransformer(vocab_size=vocab_size, d_model=d_model, pad_idx=pad_idx, nhead=nhead, enc_layers=enc_layers, dim_feedforward=dim_feedforward, dropout=dropout, emb_dropout=emb_dropout, max_len=max_len)
